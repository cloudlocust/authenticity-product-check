...
def read_article(article_id: str, db: Session = Depends(settings.get_db)):
...
def update_article(article_id: str, article: ArticleCreate, db: Session = Depends(settings.get_db)):
    """Update an article."""
...
@articles_router.delete("/{article_id}", response_model=ArticleRead)
def delete_article(article_id: str, db: Session = Depends(settings.get_db)):
    """Delete an article."""
    db_article = db.query(Article).filter(Article.id == article_id).first()
    if db_article is None:
        raise HTTPException(status_code=404, detail="Article not found")
